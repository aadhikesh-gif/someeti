...
exports.commands = {
	...
	profile: function (target, room, user) {
		...
		let profile = Db.profile.get(userid, {data: {title: {}, music: {}}});
		let avatar = (targetUser ? (isNaN(targetUser.avatar) ? `http://${serverIp}/avatars/${targetUser.avatar}` : `//play.pokemonshowdown.com/trainers-custom/${targetUser.avatar}.png`) : (Config.customavatars[userid] ? `http://${serverIp}/avatars/${Config.customavatars[userid]}` : `http://play.pokemonshowdown.com/sprites/trainers/1.png`));
		if (targetUser && targetUser.avatar[0] === "#") avatar = `//play.pokemonshowdown.com/sprites/trainers-custom/${targetUser.avatar.substr(1)}.png`;
